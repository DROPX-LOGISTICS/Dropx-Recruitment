-- Keep administrative and test locations out of public delivery applications.
create or replace function public.website_recruit_catalog(p_company uuid)
returns jsonb language sql stable security invoker set search_path = '' as $$
select jsonb_build_object(
  'locations',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'name',s.station_name,'code',s.station_code,'state',s.state) order by s.station_name)
    from public.recruitment_locations r join public.stations s on s.id=r.station_id and s.company_id=r.company_id
    where r.company_id=p_company and r.is_active and s.is_active and not s.hide_from_location_list and upper(s.station_code) !~ '^(TEST|DEMO)([^A-Z]|$)' and exists(select 1 from public.location_models lm where lm.id=s.location_model_id and lm.company_id=s.company_id and lm.is_active and lm.code<>'DROPX_HO')), '[]'::jsonb),
...
begin
 ...
 if p_kind='hr' then
   ...
   where r.id=p_location and r.company_id=p_company and r.is_active and s.is_active and not s.hide_from_location_list and upper(s.station_code) !~ '^(TEST|DEMO)([^A-Z]|$)' and exists(select 1 from public.location_models lm where lm.id=s.location_model_id and lm.company_id=s.company_id and lm.is_active and lm.code<>'DROPX_HO');
   if not found then raise exception 'not_available'; end if;
