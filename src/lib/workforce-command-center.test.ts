...
describe("workforce command center", () => {
  const adRows = [
    { adId:"1", adName:"KOZD DA A", adStatus:"active", station:"KOZD", stationName:"Kozhikode", designation:"DA", designationName:"Delivery Associate", totalLeads:18, pending:14, noStatus:12, noResponse:1, callBack:1, interviews:2, joined:1, stale24h:5 },
    { adId:"2", adName:"KOZD DA B", adStatus:"active", station:"KOZD", stationName:"Kozhikode", designation:"DA", designationName:"Delivery Associate", totalLeads:7, pending:5, noStatus:4, noResponse:1, callBack:0, interviews:1, joined:2, stale24h:2 },
  ...
  it("shows only selected ad statuses, aggregates station and designation, and sorts untreated leads first", () => {
    ...
    expect(rows[0]).toMatchObject({ station:"KOZD", designation:"DA", adCount:2, adIds:["1","2"], totalLeads:25, pending:19, noStatus:16, joined:3 });
